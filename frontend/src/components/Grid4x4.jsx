...
export default function Grid4x4({ celdas = [], estadosCeldas = {}, onClickCelda, readonly }) {
  const filas = Array.from({ length: 4 }, (_, i) => i)
  const columnas = Array.from({ length: 4 }, (_, i) => i)

  return (
    <div className={`grid-4x4 ${readonly ? 'readonly' : ''}`}>
      {filas.map((fila) =>
